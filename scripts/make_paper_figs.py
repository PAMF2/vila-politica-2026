...
def make_fig1_selective(base_p, mrp_p, ys):
    ...
    ax.legend(lines, labels, loc="lower left", fontsize=8.5,
...
def make_fig2_per_cycle(base_p, mrp_p, ys, years):
    ...
    # Acc panel
    a = axes[0]
    a.bar(x - w / 2, [v * 100 for v in base_acc], width=w,
          color="#bcd4eb", edgecolor=INK, linewidth=0.6,
          label="Cohort + Linzer baseline")
    a.bar(x + w / 2, [v * 100 for v in mrp_acc], width=w,
          color="#f2c6a2", edgecolor=INK, linewidth=0.6,
          label="v1.3 MRP ensemble")
    a.set_xticks(x)
    a.set_xticklabels([str(c) for c in cycles])
    a.set_ylim(60, 100.5)
    ...
    for xi, n in zip(x, counts):
        a.text(xi, 61.2, f"n={n}", ha="center", fontsize=7,
               color="#444", family="serif")
    _set_serif(a)

    # Brier panel
    b = axes[1]
    b.bar(x - w / 2, base_brier, width=w,
          color="#bcd4eb", edgecolor=INK, linewidth=0.6,
          label="Cohort + Linzer baseline")
    b.bar(x + w / 2, mrp_brier, width=w,
          color="#f2c6a2", edgecolor=INK, linewidth=0.6,
          label="v1.3 MRP ensemble")
    b.set_xticks(x)
    b.set_xticklabels([str(c) for c in cycles])
    b.set_ylabel("Brier (lower is better)", family="serif")
    b.set_title("Per-cycle Brier", family="serif", fontsize=11)
    b.grid(True, axis="y", linestyle=":", linewidth=0.5, alpha=0.6)
    b.set_ylim(0, max(max(base_brier), max(mrp_brier)) * 1.15)
    _set_serif(b)

    fig.suptitle("Year-fold cross-validation: baseline vs MRP-augmented blend",
                 family="serif", fontsize=11)
    plt.tight_layout()
...
def make_fig3_calibration(base_p, mrp_p, ys):
    ...
    for bins, label, color, marker in [
        (base_bins, "Cohort+Linzer baseline", COL_BASE, "o"),
        (mrp_bins, "v1.3 MRP ensemble", COL_MRP, "s"),
    ]:
        xs = [b["p_mean"] for b in bins]
        ys_ = [b["y_rate"] for b in bins]
        sizes = [max(20, 4 * b["n"]) for b in bins]
        ax.scatter(xs, ys_, s=sizes, color=color, alpha=0.75, marker=marker,
                   edgecolor="white", linewidth=0.5, label=label)
        ax.plot(xs, ys_, color=color, linewidth=1.0, alpha=0.5)
...
def make_fig7_cross_country():
    ...
    for yi, n, accm in zip(y, ns, mrp_accs):
        ax.text(108, yi, f"n={n:,}", va="center", fontsize=7.5,
